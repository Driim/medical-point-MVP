...
async def change_parent_organization(
    client: TestClient, worker_id: str, new_parent_id: str, user: str
) -> Response:
    return await client.patch(
        f"/workers/{worker_id}/change-organization/{new_parent_id}",
...
class TestOutletChangeParent:
    ...
    async def test_with_root_access_can_change_parent(
        ...
        result = await change_parent_organization(
            client, child_ou_worker, other_child_ou, user_id_with_root_access
        )
